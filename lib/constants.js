...
// --- SHARED CONSTANTS ---
// Centralizing URLs and other constants into a single file is a best practice.
// It prevents hardcoding values in multiple components, which would make them
// difficult to find and update. If a URL changes, you only need to edit it here.
...
// --- DATE UTILITIES ---
// These functions handle all date-related logic for the cohort calendar.

/**
 * Calculates the ISO week number of a given date. This is a helper function used
 * to determine the schedule for the bi-weekly accelerator program.
...
/**
 * Generates the next sprint dates dynamically. Instead of a static list, this function
 * calculates upcoming sprint dates from the current date. This makes the calendar
 * "evergreen" and removes the need for manual updates each month.
 * Sprints are on Mondays, Wednesdays, and Fridays for the next 3 months.
 * @returns {Date[]} An array of Date objects for the upcoming sprints.
 */
export const getNextSprintDates = () => {
    const dates = [];
    let currentDate = new Date(); // Start from today's date
    const endDate = new Date();
    endDate.setMonth(endDate.getMonth() + 3); // Look 3 months into the future

    while (currentDate <= endDate) {
        const dayOfWeek = currentDate.getDay();
        // Day numbers: 1=Monday, 3=Wednesday, 5=Friday
        if (dayOfWeek === 1 || dayOfWeek === 3 || dayOfWeek === 5) {
            dates.push(new Date(currentDate.setHours(19, 0, 0, 0))); // Set time to 7 PM IST
        }
...
/**
 * Generates the next accelerator dates dynamically. This function ensures the
 * accelerator cohorts, which run on alternate weekends, are always up-to-date.
...
export const getNextAcceleratorDates = () => {
    ...
    // Logic to find the next valid Saturday for the cohort
    ...
    // If the next Saturday is in an even week, we skip to the following Saturday.
    ...
    while (currentDate <= endDate) {
        ...
        saturday.setHours(10, 0, 0, 0); // 10 AM IST
        const sunday = new Date(saturday);
        sunday.setDate(saturday.getDate() + 1);
        sunday.setHours(19, 0, 0, 0); // 7 PM IST
...
/**
 * Formats a sprint date for display in a user-friendly format.
...
// --- STATIC DATA ---
// Keeping static content like course details, FAQs, and testimonials in this file
// makes the components cleaner and separates data from presentation logic.
// This is much more maintainable than hardcoding this text inside JSX.
...
// SVG mascots are defined here as React components for easy use in the UI.
...
// A reusable Testimonial Card component. While it's a component, it's highly coupled
// with the `testimonials` data structure, so keeping it here makes sense for organization.
