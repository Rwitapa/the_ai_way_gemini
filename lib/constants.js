...
export const TestimonialCard = ({ testimonial, ...rest }) => (
    <div {...rest} className="w-full bg-gray-900/ ৫০ backdrop-blur-sm rounded-2xl border border-purple-800/20 p-6 shadow-2xl shadow-purple-900/20 mb-6">
        <p className="italic text-gray-300 mb-4 text-base">"{testimonial.quote}"</p>
        <div className="flex items-center">
             {/* --- START OF THE CHANGE --- */}
             {/* Increased margin from mr-4 to mr-6 for more space */}
             <img src={testimonial.avatar} alt={`Avatar of ${testimonial.name}`} className="rounded-full mr-6 h-12 w-12 object-cover" />
             {/* --- END OF THE CHANGE --- */}
            <div>
