...
const App = () => {
  ...
  // UseEffect for Intersection Observer API to handle scroll animations
  useEffect(() => {
    // 1. Select all elements with the .animate-on-scroll class
    const scrollElements = document.querySelectorAll('.animate-on-scroll');

    if (!scrollElements.length) return;

    // 2. Create a single IntersectionObserver for performance
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        // If the element is intersecting the viewport
        if (entry.isIntersecting) {
          // 4. Add the 'is-visible' class to trigger the CSS animation
          entry.target.classList.add('is-visible');
          // 5. Unobserve the element so the animation only runs once
          observer.unobserve(entry.target);
        }
      });
    }, {
      // 3. The observer should trigger when an element is at least 10% visible
      threshold: 0.1
    });
  ...
  }, [showCoursesPage]); // Re-initialize the observer if the page view changes
  ...
  const currentPageComponent = showCoursesPage ? (
    ...
    <>
      ...
      <section 
        ...
        className="py-16 md:py-20 bg-gray-900 rounded-t-[50px] md:rounded-t-[100px] shadow-inner-xl animate-on-scroll"
      >
  ...
  <div className="container mx-auto px-6">
    ...
    >
      ...
      <section 
        ...
        className="relative py-16 md:py-20 bg-gray-900 rounded-t-[50px] md:rounded-t-[100px] shadow-inner-xl overflow-hidden animate-on-scroll"
      >
      ...
      <section 
        ...
        className="py-16 md:py-20 bg-gray-950 animate-on-scroll overflow-hidden"
      >
        <div className="container mx-auto px-6 text-center">
