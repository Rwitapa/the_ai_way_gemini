...
// This component serves as the single source of truth for all SVG icons in the application.
// By centralizing them, we achieve several key benefits for a production environment:
// 1.  Maintainability: If an icon needs to be updated, it only has to be changed in this one file.
// 2.  Consistency: Ensures all icons are used with consistent styling and properties.
// 3.  Reduced Code Duplication: Prevents the same SVG code from being scattered across multiple components.
// 4.  Performance: In-lining SVGs like this can be more performant than loading multiple small image files.

export const Icon = ({ name, size = 24, strokeWidth = 2, className = '' }) => {
    // The 'icons' object acts as a library or dictionary. The 'name' prop is used as a key
    // to retrieve the corresponding SVG element.
    const icons = {
        bolt: (
    ...
    // If an icon with the given 'name' exists in our library, we return it.
    // Otherwise, we return null to avoid rendering errors.
